...
3. Brainstorm component sources with `python ../bc-design/scripts/components.py <components>`: keep and restyle, the installed library's primitive, or an upgrade from a newer library. Apply the best option per component and name the alternatives in the handoff; ask first only when an option adds a package or is paid. On shadcn/ui, apply `../bc-design/assets/components/shadcn-bc-theme.css`.
...
- Do not change routes, data, navigation, or semantics during a restyle without approval. Visual changes need none beyond the request.
