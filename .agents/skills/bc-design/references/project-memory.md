...
2. **Existing decisions next.** The scan reports the framework, font stack, palette (`:root` properties, Tailwind `@theme` or config, DTCG token files), spacing scale, and motion libraries, each with a `file:line` citation.
