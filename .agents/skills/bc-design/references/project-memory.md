...
2. **Existing decisions next.** The scan reports the framework, font stack, palette (`:root` properties, Tailwind `@theme` or config, DTCG token files), spacing scale, motion libraries, and component libraries (shadcn/ui with its installed components and extra registries, Radix, Headless UI, MUI, and others), each with a `file:line` citation. Installed components are reused and restyled before anything new is added.
