...
| **Motion** | Transitions and movement that explain change | CSS on BC tokens for feedback (150–250ms); GSAP timelines for sequences, scroll, and 3D; a 500–750ms reveal tier only for content that appears once. Explains state, never decorates. Reduced motion always supported. | Audit: `motion-duration-budget`, `motion-easing-token`, `reduced-motion-support`, `streaming-layout-animation`, `gsap-reduced-motion`, `gsap-layout-property`. Render: reduced-motion screenshots. |
