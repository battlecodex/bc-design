...
8. **Implement in small slices.** Before building components, run `components.py` for the components in scope and offer the options: keep and restyle, the installed library's primitive, or an upgrade from a newer library (see [third-party-components.md](./third-party-components.md)). Recommend one per component and install or paste only what the user approves. For React projects without a component base, offer shadcn/ui themed with `assets/components/shadcn-bc-theme.css`. Update shared tokens and components before one-off screens. Write a failing test for behavior changes, implement the smallest change, and rerun the relevant suite. Keep streamed-text geometry stable.
