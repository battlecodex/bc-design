...
| --- | --- | --- |
| Greenfield | A new page, product, component, or design system has no visual baseline | Recorded design contract, implementation plan, and shipped interface |
...
Each mode uses the same quality gate: baseline, a recorded design contract, implementation evidence, and verification report.

An explicit request to build, redesign, or restyle is approval to change the visuals, so the contract is recorded and acted on in the same turn, not sent for sign-off. Approval is needed only before adding a package, changing behavior or content, deleting files, or using a paid or unclearly licensed asset (see "Act on explicit requests" in `SKILL.md`).
...
4. **State the direction and proceed.** Note the mode, key decisions, and scope, then implement. Stop for approval only for the gated items above; ask them together, once, and keep working on the rest.
...
8. **Implement in small slices.** Before building components, run `components.py` for the components in scope and choose per component: keep and restyle, the installed library's primitive, or an upgrade from a newer library (see [third-party-components.md](./third-party-components.md)). Apply the best option directly when it adds no package; ask before adding one. For React projects without a component base, recommend shadcn/ui themed with `assets/components/shadcn-bc-theme.css` (adding it is a package decision). Update shared tokens and components before one-off screens. Write a failing test for behavior changes, implement the smallest change, and rerun the relevant suite. Keep streamed-text geometry stable.
...
- a proposed change alters behavior, content, navigation, data, or accessibility semantics beyond what the user asked for;
- the work needs a new package, a paid component, or a file deletion;
...
- a quick pass would skip the design contract or the verification gate.
...
4. Record a contract covering information hierarchy, responsive layout, semantic tokens, states, motion, accessibility, and copy, then build it.
...
Minimum output: baseline brief, recorded design contract, implementation plan when needed, implementation, and verification report.
...
4. Record the redesign contract with an exact change list and implement it. Visual changes proceed on the user's request; changes to behavior, content, or dependencies need approval first.
...
5. Do not redesign while reviewing. If the user then asks for fixes, record a redesign or restyle contract that preserves the evidence and implement it under the same rules as any explicit request.
