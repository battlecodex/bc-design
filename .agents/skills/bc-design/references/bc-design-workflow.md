...
8. **Implement in small slices.** Reuse components in this order: the project's own components, then primitives from a component library the project already installs (reported by pre-flight), then a new library only when the user names it (see [third-party-components.md](./third-party-components.md)). Never add a UI library on your own initiative. Update shared tokens and components before one-off screens. Write a failing test for behavior changes, implement the smallest change, and rerun the relevant suite. Keep streamed-text geometry stable.
