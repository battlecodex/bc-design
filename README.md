...
- **Pre-flight.** `project.py preflight` reads a root `DESIGN.md` first, then reports the fonts, palette, spacing, motion libraries, component libraries (shadcn/ui with its installed components and registries, Radix, MUI, and others), and framework the project already uses, with `file:line` citations, so existing decisions are preserved instead of overwritten. Installed components are reused and restyled first; a new library such as shadcnblocks, 21st.dev, or React Bits is used only when the user names it. Results are cached in `.bc-design/preflight.json`.
